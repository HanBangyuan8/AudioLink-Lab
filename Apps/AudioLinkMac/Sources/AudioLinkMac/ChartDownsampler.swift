...
enum ChartDownsampler {
    static func reduce(
        ...
        let grouped = Dictionary(grouping: records, by: \.audioPathName)
        let perAudioPathLimit = max(minimumPointsPerSeries, maxTotalPoints / max(grouped.count, 1))

        return grouped.values
            .flatMap { reduceSingleAudioPath(Array($0), maxPoints: perAudioPathLimit) }
    ...
    static func reduceAlignedBatches(
        ...
        let batches = Dictionary(grouping: records, by: \.timestamp)
            .map { timestamp, records in
                MeasurementRecordBatch(timestamp: timestamp, records: records)
            }
            .sorted { $0.timestamp < $1.timestamp }

        guard batches.count > maxBatchCount else {
            return batches.flatMap(\.records).sorted(by: timestampThenAudioPath)
        }

        let reducedBatches = reduceBatches(batches, maxBatchCount: maxBatchCount)
        return reducedBatches.flatMap(\.records).sorted(by: timestampThenAudioPath)
    }

    private static func reduceSingleAudioPath(_ records: [MeasurementRecord], maxPoints: Int) -> [MeasurementRecord] {
        ...
        var buckets = Array(repeating: [MeasurementRecord](), count: bucketCount)
        ...
        var selected: [UUID: MeasurementRecord] = [:]
    ...
    private static func reduceBatches(_ batches: [MeasurementRecordBatch], maxBatchCount: Int) -> [MeasurementRecordBatch] {
        ...
        let duration = max(end.timeIntervalSince(start), 1)
        let bucketSize = max(duration / Double(bucketCount), 0.001)
        var buckets = Array(repeating: [MeasurementRecordBatch](), count: bucketCount)
        ...
        var selected: [Date: MeasurementRecordBatch] = [:]
    ...
    private static func keep(_ record: MeasurementRecord?, in selected: inout [UUID: MeasurementRecord]) {
        guard let record else { return }
        selected[record.id] = record
    }

    private static func keep(_ batch: MeasurementRecordBatch?, in selected: inout [Date: MeasurementRecordBatch]) {
        guard let batch else { return }
        selected[batch.timestamp] = batch
    }

    private static func latencyAscending(_ lhs: MeasurementRecord, _ rhs: MeasurementRecord) -> Bool {
    ...
    private static func batchLatencyAscending(_ lhs: MeasurementRecordBatch, _ rhs: MeasurementRecordBatch) -> Bool {
    ...
    private static func timestampThenAudioPath(_ lhs: MeasurementRecord, _ rhs: MeasurementRecord) -> Bool {
        if lhs.timestamp == rhs.timestamp {
            return lhs.audioPathName < rhs.audioPathName
        }
        return lhs.timestamp < rhs.timestamp
    }
}

private struct MeasurementRecordBatch {
    let timestamp: Date
